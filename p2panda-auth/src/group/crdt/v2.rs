        ...
        for (member, access) in group_state.access_levels() {
            match member {
                GroupMember::Individual(id) => {
                    ...
                    members
                        ...
                        .and_modify(|current_access| {
                            ...
                            match &root_access {
                                Some(root_access) => {
                                    if access > *current_access && access <= root_access.clone() {
                                        *current_access = access.clone()
                                    }
                                }
                                None => {
                        ...
                        .or_insert_with(|| match &root_access {
                            Some(root_access) => {
                                if access <= root_access.clone() {
                                    access
                                } else {
                                    root_access.clone()
                                }
                ...
                GroupMember::Group(id) => {
                    ...
                    match root_access {
                        Some(ref root_access) => {
                            self.members_inner(id, members, Some(root_access.clone()))
                        }
                        None => self.members_inner(id, members, Some(access)),
                    }
                }
            }
        }
...
pub(crate) mod tests {
    ...
    pub(crate) fn sync(y: TestGroupState, ops: &[TestOperation]) -> TestGroupState {
        ops.iter()
            .fold(y, |g, op| TestGroup::process(g, op).unwrap())
    }
