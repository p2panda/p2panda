...
/// Modify the state a group member by applying the given access level.
///
/// The actor performing the modification must be an active member of the group with manager access.
/// The modified identity must also be an active member of the group. Failure to meet these
/// conditions will result in an error.
///
/// No modification will occur if the member already has `Manage` access. In that case, the given
...
///
/// The actor performing the promotion must be an active member of the group with manager access.
/// The promoted identity must also be an active member of the group. Failure to meet these
/// conditions will result in an error.
///
/// No modification will occur if the promoted member already has `Manage` access. In that case,
/// the given state is returned unchanged.
...
mod tests {
    ...
    fn promote_demote_modify() {
        ...
        let group_y = demote(group_y.clone(), alice, bob, AccessLevel::Read).unwrap();
    ...
    fn demote_errors() {
        ...
        // Daphne demotes Charlie to Pull access...
        let result = demote(group_y.clone(), daphne, charlie, AccessLevel::Pull);
        ...
        // Bob demotes Charlie to Pull access...
        ...
        // Alice demotes Daphne to Read access...
        ...
        // Alice demotes Charlie to Pull access...
        ...
        // Charlie demotes Bob to Pull access...
        let result = demote(group_y, charlie, bob, AccessLevel::Pull);
    ...
    fn merge_state_access_counter() {
        ...
        // Alice promotes Charlie to Read access.
