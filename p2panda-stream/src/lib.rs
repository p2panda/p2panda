...
//! protocol, LoRa, etc.). Ideally the application should just work fine independent of how the data
//! was delivered, the only part which really matters is how the event stream was _processed_.
//!
//! This stream processor design can be nicely combined with a Pub/Sub system (like `p2panda-net`),
//! scoped and stateful stream controllers (in `p2panda` Node API) and storage backends supporting
