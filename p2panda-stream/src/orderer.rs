...
mod tests {
    ...
    async fn partial_order() {
        assert_all_stores!(|store| async {
            ...
            assert_eq!(checker.store.ready_len().await, 1);
            assert_eq!(checker.store.pending_len().await, 0);
            assert_eq!(checker.store.ready_queue_len().await, 1);

            // B has it's dependencies met and so it too is added to the processed set and ready
            // queue.
            let item = graph[1].clone();
            checker.process(item.0, &item.1).await.unwrap();
            assert_eq!(checker.store.ready_len().await, 2);
            assert_eq!(checker.store.pending_len().await, 0);
            assert_eq!(checker.store.ready_queue_len().await, 2);

            // D doesn't have both its dependencies met yet so it waits in the pending queue.
            let item = graph[3].clone();
            checker.process(item.0, &item.1).await.unwrap();
            assert_eq!(checker.store.ready_len().await, 2);
            assert_eq!(checker.store.pending_len().await, 1);
            assert_eq!(checker.store.ready_queue_len().await, 2);
            ...
            checker.process(item.0, &item.1).await.unwrap();
            assert_eq!(checker.store.ready_len().await, 4);
            assert_eq!(checker.store.pending_len().await, 0);
            assert_eq!(checker.store.ready_queue_len().await, 4);
    ...
    async fn partial_order_with_recursion() {
        assert_all_stores!(|store| async {
            ...
            assert_eq!(checker.store.ready_len().await, 1);
            assert_eq!(checker.store.pending_len().await, 5);
            assert_eq!(checker.store.ready_queue_len().await, 1);
            ...
            checker
                ...
                .await
                .unwrap();
            assert_eq!(checker.store.ready_len().await, 7);
            assert_eq!(checker.store.pending_len().await, 0);
            assert_eq!(checker.store.ready_queue_len().await, 7);
    ...
    async fn complex_graph() {
        assert_all_stores!(|store| async {
            ...
            assert!(checker.store.ready_len().await == 3);
            assert_eq!(checker.store.pending_len().await, 3);
            assert_eq!(checker.store.ready_queue_len().await, 3);
            ...
            // No more ready items.
            assert_eq!(checker.store.ready_queue_len().await, 0);
