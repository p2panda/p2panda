...
#[test]
fn missing_key_bundle_on_process_create() {
    let rng = Rng::from_seed([1; 32]);
    ...
    // NOTE: Alice registers their own and Bob's pre-key bundles.
    let alice_pki = KeyRegistry::add_longterm_bundle(alice_pki, alice, alice_prekeys).unwrap();
    let alice_pki = KeyRegistry::add_longterm_bundle(alice_pki, bob, bob_prekeys.clone()).unwrap();

    // NOTE: Bob registers their own, but not Alice's pre-key bundle.
    // let bob_pki = KeyRegistry::add_longterm_bundle(bob_pki, alice, alice_prekeys.clone()).unwrap();
    ...
    // NOTE: Processing here causes an error "MissingPreKeys(0)"
    let (_bob_dcgka, output) = Dcgka::process(
        bob_dcgka,
        ProcessInput {
            seq: MessageId {
