...
mod tests {
    ...
    use crate::test_utils::memory_store::helpers::{
        populate_test_db, send_to_store, test_db_config, PopulateDatabaseConfig, TestStore,
    };
    ...
    async fn gets_document_id_for_view(schema: Schema, operation: Operation) {
        ...
        let (entry, _) = send_to_store(&db.store, &operation, &schema, &KeyPair::new())
            .await
            .unwrap();
        ...
        let (entry, _) = send_to_store(&db.store, &update_operation, &schema, &KeyPair::new())
            .await
            .unwrap();
    ...
    async fn publish_with_missing_operations(
        ...
        let existing_author = Author::from(db.test_data.key_pairs[0].public_key());
        ...
        let next_args = next_args(&db.store, &author_to_publish, document.as_ref())
            .await
            .unwrap();
        ...
        remove_operations(&db.store, &existing_author, operations_to_remove);
    ...
    async fn next_args_with_expected_results(
        ...
        let expected_next_args = EntryArgsResponse {
            ...
            backlink: expected_backlink.map(|hash| hash),
            skiplink: expected_skiplink.map(|hash| hash),
        };
