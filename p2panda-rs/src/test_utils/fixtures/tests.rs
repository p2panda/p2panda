...
// different `message` calue cases which are manually passed in via the #[case] macro. We can name
...
// non default message values. These are defined in fixtures/templates.rs. We also set a custom
// case which should pass.
#[apply(non_default_message_values_panic)]
#[case(defaults::create_message())]
fn message_validation_with_templates(entry: Entry, #[case] message: Message, key_pair: KeyPair) {
    let encoded_message = MessageEncoded::try_from(&message).unwrap();
    let signed_encoded_entry = sign_and_encode(&entry, &key_pair).unwrap();
    assert!(signed_encoded_entry
        .validate_message(&encoded_message)
