...
pub(crate) fn typed_members<C: Conditions>(
    y: &AuthGroupState<C>,
    members: Vec<(VerifyingKey, Access<C>)>,
) -> Vec<(GroupMember<VerifyingKey>, Access<C>)> {
    members
        .into_iter()
...
pub(crate) fn secret_members<C>(members: Vec<(VerifyingKey, Access<C>)>) -> Vec<VerifyingKey> {
    let mut members: Vec<VerifyingKey> = members
        .into_iter()
