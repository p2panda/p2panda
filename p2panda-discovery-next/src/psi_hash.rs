...
pub const ALICE_SALT_BYTE: u8 = 0;
pub const BOB_SALT_BYTE: u8 = 1;

/// PSI protocol message
///
/// Alice (the initiator) and Bob (the accepter) exchange these messages in the order they are written here.
/// Any out of order message recieved should result in the protocol returning an error.
...
///
/// PSI is a method for determining commonly held information between parties without revealing the actual values.
/// Here this is done by exchanging hashed versions of the topics so that each peer can compute the intersection locally.
/// At the end of a sucessful run both peers should end up with the same set of intersecting topics.
///
/// We generate a salt that is unique per session by concatenating [random bytes][`generate_salt_half`] from both peers and use it in the hash to prevent replay attacks.
/// Additionally we add a single constant byte to the salt depending on which peer the message is coming from so that bob cannot simply replay alice's hashes as his own answer.
...
/// Concatinates the two salt halves generated by each peer into a single salt unique for this session.
///
/// We use [ALICE_SALT_BYTE] or [BOB_SALT_BYTE] to make the hashing unique in both directions.
/// If alice and bob used the exact same salt bob could fool alice by simply replaying alices hashes back to her.
pub fn combine_salt(
...
/// Protocol error
///
/// Fatal errors returned by the protocol
#[derive(Debug, Error)]
