        ...
        match message {
            ...
            ToSyncManager::InitiateSync(topic, node_id) => {
                ...
                if !state.authoriser.can_connect_on_topic(topic, node_id).await {
                    warn!("blocked initiating sync with node {} on topic {}", node_id.fmt_short(), topic.fmt_short());
                    return Ok(());
                }
    ...
    E: Send + 'static,
{
    async fn accept(
        ...
        if !allow {
            warn!("rejected sync with blocked node {} on topic {}", node_id.fmt_short(), topic.fmt_short());
