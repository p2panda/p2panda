...
//! ### Access Control
//!
//! Nobody should be able to sync data without our permission. How this is enforced depends on the
//! underlying transport.
//!
//! Typical approaches include capability-based authorization, where presenting a token grants sync
//! access to the requested data, or an access-control list managed by a CRDT or a user-managed
//! database. These approaches generally assume that a confidential channel has been established
//! between the parties. In store-and-forward contexts, we may instead need to rely more heavily on
//! group encryption.
//!
//! ### Fast-push vs. sync
//!
