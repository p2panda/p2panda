...
//!
//! For this we need to define three things:
//!
...
//!    group.
//! 2. When an "add", "update" or "remove" control message is created, the message needs to point
//!    at a) the last, previously processed control messages (by us and others), b) if any
//!    application messages were sent by us, the last sent message. This helps with peers
//!    understanding that they might miss a message when they switch to a new ratchet, they can
//!    decide to ignore this message "dependency", but will also then potentially loose that
//!    message. This can be useful to do if messages get lost and peers otherwise get "stuck".
...
//! 3. All of this processing needs to be done before we can move on processing future messages.
//!
//! In the given example "Charlie" would be added to the group by Bob's "add" control message.
//! Charlie would process the "welcome", acknowledge it and look at all messages now. They
