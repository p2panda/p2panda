...
pub fn message(
    ...
    match fields {
        ...
        Some(fields) if instance_id.is_none() => Panda::create_message(CHESS_SCHEMA, fields),
        // It's an UPDATE message
        Some(fields) => Panda::update_message(CHESS_SCHEMA, instance_id.unwrap(), fields),
        // It's a DELETE message
        None if instance_id.is_some() => Panda::delete_message(CHESS_SCHEMA, instance_id.unwrap()),
        // It's a mistake....
        None => todo!() // Error.... 
    }
}
...
pub fn v0_1_0_fixture() -> Fixture {
    Fixture {
        ...
        entry: Panda::entry(&Panda::create_message(CHESS_SCHEMA, vec![("name", "chess"), ("description", "for playing chess")]), &Panda::seq_num(1), None, None)
    }
}
